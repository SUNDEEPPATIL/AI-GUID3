...
const ProductModal: React.FC<ProductModalProps> = ({ product, category, onClose, isWishlisted, onToggleWishlist, onShare }) => {
  ...
  useEffect(() => {
    ...
    const getAnalysis = async () => {
      ...
      try {
        ...
        addToast(error.message || 'Failed to load analysis', 'error');
      } finally {
  ...
  const price = (product as any).priceInINR || (product as any).expectedPriceInINR;
  const launchInfo = (product as any).launchYear || (product as any).expectedLaunch;
  ...
  const uniqueId = ((product as any).modelName || (product as any).id || 'product').toString().replace(/\s+/g, '-');
  ...
  const renderInfoItem = (Icon: React.FC<any>, label: string, value?: string | number) => {
    ...
    return (
      <div className="flex items-center gap-2 text-sm text-gray-300">
        ...
        <span>{label}: <span className="font-semibold text-white">{(value as any).toLocaleString ? (value as any).toLocaleString() : value}</span></span>
      </div>
    );
  };
  ...
  const visualSpecs = allSpecs.filter(spec => {
    const value = (product as any)[spec.key as keyof Product];
   ...
   >
     ...
     >
       ...
       <header className="sticky top-0 bg-gray-800/80 backdrop-blur-sm z-10 p-4 border-b border-gray-700 flex justify-between items-start gap-4">
         <div>
           <h2 id="product-modal-title" className="text-xl font-bold text-white">{(product as any).brand} {(product as any).modelName}</h2>
           <div className="flex items-center gap-3 mt-1">
             <StarRating rating={(product as any).reviewStars} />
             {(product as any).totalReviews && <span className="text-xs text-gray-400">({(product as any).totalReviews.toLocaleString?.() || (product as any).totalReviews} reviews)</span>}
           </div>
         </div>
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
           ...
           <div className="space-y-6">
             <section className={cardClasses} aria-labelledby={keyInfoId}>
               ...
               <div className="space-y-2">
                 {renderInfoItem(RupeeIcon, (product as any).priceInINR ? 'Price' : 'Expected Price', price)}
                 {renderInfoItem(CalendarIcon, (product as any).launchYear ? 'Launched' : 'Expected Launch', launchInfo)}
                  <p className="text-sm text-gray-400 pt-2">{(product as any).summary}</p>
               </div>
             </section>
             
             {(visualSpecs.length > 0 || ((product as any).keySpecs && (product as any).keySpecs.length > 0)) && (
               <section className={cardClasses} aria-labelledby={keySpecsId}>
                 ...
                 {visualSpecs.length > 0 && (
                   <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-center">
                     {visualSpecs.map(({ key, label, icon: Icon }) => {
                       const value = (product as any)[key as keyof Product];
                       return (
                         <div key={key} className="flex flex-col items-center p-2 bg-gray-800/60 rounded-lg">
                           <Icon className="w-4 h-4 text-cyan-400 mb-2" />
                           <p className="text-sm font-semibold text-white truncate w-full">{Array.isArray(value) ? value.join(', ') : (value?.toLocaleString ? value.toLocaleString() : value)}</p>
                 ...
                 {(product as any).keySpecs && (product as any).keySpecs.length > 0 && (
                   <div className={`space-y-2 ${visualSpecs.length > 0 ? 'pt-4 mt-4 border-t border-gray-700/50' : ''}`}> 
                     {(product as any).keySpecs.map((spec: string, index: number) => (
             ...
             {(product as any).retailerPrices && (product as any).retailerPrices.length > 0 && (
               <section className={cardClasses} aria-labelledby={whereToBuyId}>
                 ...
                 <div className="space-y-2">
                   {(product as any).retailerPrices.map((retailer: RetailerPrice) => (
           ...
           <div className="space-y-6">
             <section className={cardClasses} aria-labelledby={aiAnalysisId}>
              ...
              {(product as any).userReviews && (product as any).userReviews.length > 0 && (
               <section className={cardClasses} aria-labelledby={userReviewsId}>
                 ...
                 <div className="space-y-3">
                   {(product as any).userReviews.map((review: any, i: number) => (
